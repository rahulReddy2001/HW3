...
class BTreeIndex {
 ...
 public:
  ...
  void findPageForScan();

  bool isLeafFull(LeafNodeInt* leaf);

  bool isLeafEmpty(LeafNodeInt* leaf);

  int nonLeafPageLen(NonLeafNodeInt *nonLeaf);

  int leafPageLen(LeafNodeInt *leaf);
  
  int findArrayIndex(const int *arr, int key, bool gte);
